...
dc.rowChart = function (parent, chartGroup) {
    ...
    var _labelOffsetY = 10;
	//var _labelOffsetY = height/2;
    ...
    function updateLabels(rows) {
		var n = _rowData.length; //Kyaw: added for text y position calculation
        var height = (_chart.effectiveHeight() - (n + 1) * _gap) / n; //Kyaw:added for text y position calculation
        if (_chart.renderLabel()) {
            var lab = rows.select("text")
                .attr("x", _labelOffsetX)
               // .attr("y", _labelOffsetY)
				.attr("y", height/2 +0) //Kyaw: added for text y position calculation
				.attr("dy", "0.35em") //Kyaw: added for text y position calculation
                .on("click", onClick)
                .attr("class", function (d, i) {
                    return _rowCssClass + " _" + i;
                })
